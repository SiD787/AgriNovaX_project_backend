...
async def predict(request: PredictRequest):
    ...
    # 2. Hard Filtering (reject if highly unsuitable for location/weather)
    sorted_crops = prediction["sorted_crops"]
    valid_crop = None
    
    # Iterate through top crops to find one that fits the environment
    alternatives = []
    
    for c_data in sorted_crops:
        crop_candidate = c_data["crop"]
        conf = c_data["confidence"]
        
        # Check suitability
        p_table = match_parameters(
            crop_candidate, request.N, request.P, request.K,
            request.temperature, request.humidity, request.ph, request.rainfall
        )
        r_level = assess_risk(p_table)
        
        # We want to avoid crops with severe weather mismatch ("High" risk)
        # Unless it's our only option left
        if r_level != "High" and conf > 1.0:
            if valid_crop is None:
                valid_crop = crop_candidate
                model_confidence = conf
                parameter_table = p_table
                risk_level = r_level
        ...
        if valid_crop is not None and len(alternatives) == 3:
            break
            
    # Fallback to the absolute highest probability crop if everything is High Risk
    if valid_crop is None:
        valid_crop = sorted_crops[0]["crop"]
        model_confidence = sorted_crops[0]["confidence"]
        parameter_table = match_parameters(
            valid_crop, request.N, request.P, request.K,
            request.temperature, request.humidity, request.ph, request.rainfall
        )
        risk_level = assess_risk(parameter_table)
        # Fill alternatives with next best
        alternatives = [{"crop": c["crop"], "confidence": c["confidence"]} for c in sorted_crops[1:4]]
        
    best_crop = valid_crop
...
class ChatRequest(BaseModel):
    ...
    context: Optional[dict] = Field(None, description="Current analysis results context")
